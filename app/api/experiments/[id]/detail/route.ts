...
export async function GET(
  ...
  try {
    ...
    const { data: experiment, error: expErr } = await dbClient
      .from("experiments")
      .select("id, number, repetition_count, test_count, start_date, strain, fungus_id, strain_acronym, strain_variable, strain_observation, status, canceled_at, canceled_by")
      .eq("id", experimentId)
      .maybeSingle()
