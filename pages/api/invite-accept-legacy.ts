...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    if (!token) {
      return jsonError(res, "Token do convite não informado.", 400)
    }

    if (!password || password.length < 6) {
      return jsonError(res, "A senha deve ter pelo menos 6 caracteres.", 400)
    }

    if (password !== repeatPassword) {
      return jsonError(res, "As senhas não coincidem.", 400)
    }

    // Usamos any aqui de propósito para não quebrar o build caso o database.types.ts
    // fique temporariamente defasado em algum deploy.
    ...
    if (invitationError) {
      return jsonError(res, invitationError.message || "Erro ao validar convite.", 500)
    }

    if (!invitation) {
      return jsonError(res, "Convite inválido.", 404)
    }
    ...
    if (acceptError) {
      return jsonError(res, acceptError.message || "Erro ao concluir convite.", 500)
    }

    // Esta rota fallback prioriza concluir o cadastro com estabilidade.
    // Depois da criação, enviamos para o login para o usuário entrar com a senha recém-criada.
