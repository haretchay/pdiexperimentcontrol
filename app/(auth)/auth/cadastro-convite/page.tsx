...
import { KeyRound, Loader2, Lock, Mail, ShieldCheck, XCircle } from "lucide-react"
...
export default function CadastroConvitePage() {
  ...
  useEffect(() => {
    ...
    async function validateToken() {
      ...
      try {
        const response = await fetch(`/api/pdi/validate-invite?token=${encodeURIComponent(token)}&v=20260523`, {
          cache: "no-store",
        })
        ...
        if (mounted) setInvitation(payload.invitation)
      } catch (err) {
        if (mounted) {
          setInvitation(null)
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/pdi/accept-invite?v=20260523", {
        method: "POST",
        headers: {
          ...
          <CardContent className="space-y-6">
            {isValidating ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {error && (
                  <Alert className="border-red-300/40 bg-red-500/15 text-red-50">
                ...
                <Button
                  ...
                  className="h-12 w-full rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 text-base font-semibold text-white shadow-lg hover:from-blue-600 hover:to-purple-700"
                >
                  {isSubmitting ? (
                    <>
